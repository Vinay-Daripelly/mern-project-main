...
const Profile = () => {
  ...
  const handleFinish=async(values)=>{
    try{
     ...
     const res=await axios.post('/api/v1/trainer/updateProfile',{...values,userId:user._id,
      timings:[
  ...
  const getTrainerInfo=async()=>{
    try{
      const res=await axios.post('/api/v1/trainer/getTrainerInfo',
      {userId:params.id},
    {
      headers:{
        Authorization:`Bearer ${localStorage.getItem('token')}`
    ...
    <Layout>
        ...
        {trainer && (
          <Form layout="vertical" onFinish={handleFinish} className='m-3' initialValues={{
            ...trainer,
            timings:[
              moment(trainer.timings[0],'HH:mm'),
              moment(trainer.timings[1],'HH:mm'),
            ]
          }}>
